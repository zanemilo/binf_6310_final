#!/usr/bin/env bash
#SBATCH --job-name=ildiko_pipeline
...
#SBATCH --output=/scratch/polyak.i/ildiko_pipeline_automatic/logs/pipeline_%j.log

export PATH=/home/polyak.i/micromamba/envs/binf6310/bin:$PATH

set -euo pipefail

# Full RNA-seq pipeline: Download → FastQC → STAR alignment → featureCounts
# Based on Liu et al. 2025 reproducibility study
# USAGE:  sbatch ildiko_pipeline_automatic.sh tip_samples.tsv
...
if [[ -z "$SAMPLES_TSV" ]]; then
    ...
    echo "Usage: bash ildiko_pipeline_automatic.sh <samples.tsv>"
    exit 1
fi
